...
const RichTextRenderer: React.FC<{ content: string; backendUrl?: string | null }> = ({ content, backendUrl }) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  ...
  const processContent = (text: string) => {
    ...
    if (backendUrl) {
      processed = processed.replace(/\[Page (\d+)\]/g, (match, page) => 
          ...
          <ContentContainer>
            <div className="space-y-2">
              {data.supporting_extracts.map((extract: any, idx: number) => (
                <RichTextRenderer key={idx} content={extract.text} backendUrl={backendUrl} />
              ))}
            </div>
          </ContentContainer>
        </div>
                              ...
                              {data.citations.map((citation, index) => (
                                ...
                                >
                                  <div className="flex justify-between items-start gap-3">
                                    ...
                                    <a
                                      href={`${backendUrl}/pdf/reliance/reliance_faq.pdf#page=${citation.page}`}
                                      target="_blank"
                              ...
                              {data.relevant_paragraphs.map((para, index) => (
                                ...
                                >
                                  <div className="flex justify-between items-start mb-3">
                                    ...
                                    <a
                                      href={`${backendUrl}/pdf/reliance/reliance_faq.pdf#page=${para.page}`}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className={`text-sm font-medium px-2 py-1 rounded transition-colors ${
                                        isDark 
